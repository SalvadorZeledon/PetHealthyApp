...
const VetConsultationScreen = ({ navigation, route }) => {
  ...
  const [selectedDewormType, setSelectedDewormType] =
    useState(DEWORM_TYPES[0]);
  ...
  const handleSaveVaccine = () => {
    ...
    // 🔴 NUEVO: validar marca y lote obligatorios
    if (!vaccineBrand.trim() || !vaccineSerial.trim()) {
      Dialog.show({
        type: ALERT_TYPE.WARNING,
        title: "Datos incompletos",
        textBody: "Marca y lote de la vacuna son obligatorios.",
        button: "Corregir",
      });
      return;
    }
    ...
    if (isFutureDate(vaccineDate)) {
      Dialog.show({
        ...
        textBody: "La fecha de aplicación no puede ser futura.",
        button: "Entendido",
      });
      return;
    }
    if (isFutureDate(vaccineMfgDate)) {
      Dialog.show({
        type: ALERT_TYPE.WARNING,
        title: "Fecha Incorrecta",
        textBody: "La fecha de fabricación no puede ser futura.",
        button: "Entendido",
      });
      return;
    }
    if (!isFutureDate(vaccineExpDate)) {
      Dialog.show({
        ...
        textBody: "La fecha de vencimiento debe ser futura.",
        button: "Entendido",
      });
      return;
    }
    if (vaccineMfgDate >= vaccineExpDate) {
      Dialog.show({
        type: ALERT_TYPE.WARNING,
        title: "Incoherencia",
        textBody: "La fecha de fabricación debe ser anterior al vencimiento.",
        button: "Corregir",
      });
      return;
    }
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        if (file.size && file.size > MAX_FILE_SIZE) {
          Dialog.show({
            ...
            textBody: "El PDF supera los 10MB permitidos.",
            button: "Entendido",
          });
          return;
        }
        setAttachments([
          ...attachments,
          { uri: file.uri, type: "pdf", name: file.name },
        ]);
      }
    } catch (err) {
      console.log(err);
      ...
      if (!result.canceled && result.assets.length > 0) {
        ...
        if (asset.fileSize && asset.fileSize > MAX_FILE_SIZE) {
          Dialog.show({
            ...
            textBody: "La foto supera los 10MB permitidos.",
            button: "Entendido",
          });
          return;
        }
  ...
  const handleFinalSave = async () => {
    if (!reason.trim() || !diagnosis.trim() || !vetName.trim()) {
      Dialog.show({
        type: ALERT_TYPE.WARNING,
        title: "Faltan datos",
        textBody: "Motivo, Diagnóstico y Nombre del médico son obligatorios.",
        button: "Ok",
      });
      return;
    }
    ...
    try {
      ...
      for (const file of attachments) {
        ...
        if (url)
          uploadedFiles.push({ url, type: file.type, name: file.name });
      }
      ...
      await addDoc(
        collection(db, COL_MASCOTAS, petId, "consultas"),
        consultationData
      );
      ...
      Dialog.show({
        type: ALERT_TYPE.DANGER,
        title: "Error",
        textBody: "No se pudo guardar la consulta.",
        button: "Cerrar",
      });
    } finally {
      setLoading(false);
    ...
    >
      ...
      <ScrollView contentContainerStyle={styles.content}>
          ...
          <View style={styles.rowInputs}>
            <View style={{ flex: 1, marginRight: 8 }}>
              <Text style={styles.label}>
                Nombre Médico <Text style={{ color: "red" }}>*</Text>
              </Text>
              <TextInput
                style={styles.input}
                value={vetName}
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.label}>
            Razón de la visita <Text style={{ color: "red" }}>*</Text>
          </Text>
          <TextInput
            style={styles.input}
            value={reason}
          ...
          />
          <Text style={styles.label}>
            Diagnóstico Médico <Text style={{ color: "red" }}>*</Text>
          </Text>
          <TextInput
            style={[styles.input, styles.textArea]}
            value={diagnosis}
          ...
          <View style={styles.tableContainer}>
            {vaccines.length === 0 ? (
              <View style={styles.emptyRow}>
                <Text style={styles.emptyText}>Sin vacunas hoy.</Text>
              </View>
            ) : (
        ...
        <View style={styles.card}>
          ...
          <View style={styles.tableContainer}>
            {dewormings.length === 0 ? (
              <View style={styles.emptyRow}>
                <Text style={styles.emptyText}>Sin desparasitaciones hoy.</Text>
              </View>
            ) : (
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <ScrollView>
              ...
              >
                {COMMON_VACCINES.map((v) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.chip,
                      selectedVaccine === v && styles.chipSelected,
                    ]}
                    onPress={() => setSelectedVaccine(v)}
                  >
                    <Text
                      style={{
                        color: selectedVaccine === v ? "white" : "black",
                      }}
                    >
                      {v}
                    </Text>
              ...
              <Text style={styles.label}>
                Marca <Text style={{ color: "red" }}>*</Text>
              </Text>
              <TextInput
                style={styles.input}
                value={vaccineBrand}
              ...
              />
              <Text style={styles.label}>
                Lote <Text style={{ color: "red" }}>*</Text>
              </Text>
              <TextInput
                style={styles.input}
                value={vaccineSerial}
              ...
              <View style={styles.dateButtonsRow}>
                <TouchableOpacity
                  ...
                  onPress={() => setShowVaccineDateModal(true)}
                >
                  <Text style={{ fontSize: 11 }}>
                    Aplic: {formatDate(vaccineDate)}
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.dateBtn}
                  onPress={() => setShowVaccineMfgModal(true)}
                >
                  <Text style={{ fontSize: 11 }}>
                    Fab: {formatDate(vaccineMfgDate)}
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.dateBtn}
                  onPress={() => setShowVaccineExpModal(true)}
                >
                  <Text style={{ fontSize: 11 }}>
                    Venc: {formatDate(vaccineExpDate)}
                  </Text>
                </TouchableOpacity>
              </View>
            ...
            >
              ...
              <TouchableOpacity onPress={handleSaveVaccine}>
      ...
      <Modal
        ...
        transparent
        animationType="fade"
        onRequestClose={() => setShowDewormModal(false)}
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <ScrollView>
              <View style={{ flexDirection: "row", marginBottom: 10 }}>
                {DEWORM_TYPES.map((t) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.chip,
                      selectedDewormType === t && styles.chipSelected,
                    ]}
                    onPress={() => setSelectedDewormType(t)}
                  >
                    <Text
                      style={{
                        color: selectedDewormType === t ? "white" : "black",
                      }}
                    >
                      {t}
                    </Text>
              ...
              <View style={styles.dateButtonsRow}>
                <TouchableOpacity
                  style={styles.dateBtn}
                  onPress={() => setShowDewormDateModal(true)}
                >
                  <Text style={{ fontSize: 11 }}>
                    Aplic: {formatDate(dewormDate)}
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.dateBtn}
                  onPress={() => setShowDewormMfgModal(true)}
                >
                  <Text style={{ fontSize: 11 }}>
                    Fab: {formatDate(dewormMfgDate)}
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.dateBtn}
                  onPress={() => setShowDewormExpModal(true)}
                >
                  <Text style={{ fontSize: 11 }}>
                    Venc: {formatDate(dewormExpDate)}
                  </Text>
                </TouchableOpacity>
              </View>
              {showDewormDateModal && (
            ...
            >
              <TouchableOpacity onPress={() => setShowDewormModal(false)}>
                <Text style={{ color: "red" }}>Cancelar</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={handleSaveDeworming}>
                <Text style={{ color: "#7B1FA2", fontWeight: "bold" }}>
                  Guardar
                </Text>
              </TouchableOpacity>
